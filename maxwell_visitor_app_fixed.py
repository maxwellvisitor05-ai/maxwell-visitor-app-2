...
def latest_pending():
    ...
    if host:
        row = conn.execute(
            "SELECT id,name,person_to_meet,created_at FROM visitors WHERE status='pending' AND person_to_meet=? ORDER BY id DESC LIMIT 1",
            (host,)
        ).fetchone()
    else:
        row = conn.execute(
            "SELECT id,name,person_to_meet,created_at FROM visitors WHERE status='pending' ORDER BY id DESC LIMIT 1"
        ).fetchone()
    conn.close()
